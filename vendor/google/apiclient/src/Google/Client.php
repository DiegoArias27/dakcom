...
class Google_Client
{
  ...
  public function isAccessTokenExpired()
  {
    ...
    // Verificar si la clave 'expires_in' está definida en el arreglo $this->token
if (isset($this->token['expires_in']) && isset($created)) {
    // Si está definida, verifica si el token está a punto de expirar
    $expired = ($created + ($this->token['expires_in'] - 30)) < time();
} else {
    // Si no está definida, manejamos el caso en que no se pueda calcular la expiración
    $expired = true; // O cualquier valor que prefieras para indicar que no se puede determinar la expiración
}
